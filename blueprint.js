...
var Blueprint = (function (Engines) {
  ...
  /* Today as yyyy-mm-dd in LOCAL time, so the comparison matches the date the
   * traveller sees in the canonical date picker. Built from the parts rather
   * than from toISOString(), which is UTC and would call today "the past" for
   * anyone west of Greenwich for part of the day. Ruling AM item 6. */
  function todayISO() {
    var d = new Date();
  ...
  function validate(blueprint) {
    ...
    /* RULING AM item 6. A start date in the past was accepted in SILENCE.
     * There was no past-date check of any kind: the only date rules were
     * end-after-start, the nights range, and presence.
     *
     * An ERROR, not a warning, on the end_date precedent one line above —
     * which is also what makes it free: it points at the canonical
     * s-tripdetails screen without touching a byte of it.
     *
     * NOTE WHAT THIS DOES NOT CURE, so it is not mistaken for the whole fix.
     * The trip that came back dated 2025 had NO dates set at all; its
     * traveller never walked through this door. That defect is the prompt's
     * and is fixed in liveslice-api.js. This is a second hole, found while
     * tracing the first. */
    if (bp.start_date && bp.start_date < todayISO()) {
      err('start_date', 'Your departure date is in the past. Pick a date from today onwards.', 's-tripdetails');
    }
    if (!bp.nights) {
    ...
    if (!bp.start_date || !bp.end_date) {
      /* RULING AM item 6. This said "the itinerary will be generated by day
       * number, not by date", which was FALSE: the prompt asked the model for
       * dates and the day cards printed them. It now says what actually
       * happens, which is that the bundle picks them. */
      warn('start_date', 'No travel dates set. Dates will be assumed from the number of nights.');
    }
    ...
    if (bp.has_kids && !(bp.kid_ages_months || []).length) {
      err('kid_ages_months', 'Add each child\'s age. Age limits depend on it.', 's-bp-kids');
    }
    ...
    if (!bp.pace) {
      err('pace', 'Choose a pace. It sets how full your days are.', 's-ls-detail');
    }
